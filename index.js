(function (factory) {
  ...
  function releases() {
    ...
    return fetch('data/releases.json').then(function (response) {
      ...
      var latestReleases = Object.keys(json) //identifying keys
      .map(function (key) {
        return json[key];
      }) //mapping keys to values
      .filter(function (releases) {
        return releases.length > 0;
      }) //filering out redundand objects?
      .map(function (releases) {
        ...
        })[0]; //getting the latest release for each renderer
      }).sort(function (a, b) {
        return b.date.getTime() - a.date.getTime();
      });
      return latestReleases.map(function (release) {
        return {
          name: release.repo,
  ...
  function repos() {
    ...
    return fetch('data/repos.json').then(function (response) {
      return response.json();
    }).then(function (json) {
  ...
  var releases$1 = releases();
  var repos$1 = repos();
  Promise.all([releases$1, repos$1]).then(function (arr) {
    var _arr = _slicedToArray(arr, 2),
        releases = _arr[0],
        repos = _arr[1];

    releases.forEach(function (release) {
      var repo = repos.find(function (e) {
        return e.name === release.name;
      });
      console.log(repo);
    ...
    var releasesTable = webCharts.createTable(".releases", {
      nRowsPerPage: 20,
      bootstrap: false,
      headers: ["Name", "Last Update", "Latest Release", "Open Issues"]
    });
